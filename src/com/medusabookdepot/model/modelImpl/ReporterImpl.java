...
public class ReporterImpl implements Reporter {
    ...
    @Override
    public void buildReport(int dayBegin, int monthBegin, int yearBegin, int dayEnd, int monthEnd, int yearEnd,
            String fileName) {
        List<Transfer> transes = TransferManagerImpl.getInstanceOfTransferManger().getAllTransfers();
        BufferedWriter bw;
        Calendar c1 = Calendar.getInstance();
        c1.set(yearBegin, monthBegin - 1, dayBegin, 0, 0, 0);
        Date dateBeg = c1.getTime();
        Calendar c2 = Calendar.getInstance();
        c2.set(yearEnd, monthEnd - 1, dayEnd, 23, 59, 59);
        ...
        try {
            ...
            for (Transfer t : transes) {
                if ((t.getLeavingDate().after(dateBeg) || t.getLeavingDate().equals(dateBeg))
                        && (t.getLeavingDate().before(dateEnd) || t.getLeavingDate().equals(dateBeg))) {
                    x++;
                    bw.write("trasferimento " + x + ":");
                    bw.newLine();
                    bw.write("          tracking number=" + t.getTrackingNumber());
                    bw.newLine();
                    bw.write("          prezzo totale=" + t.getTotalPrice() + " €");
                    bw.newLine();
                    bw.write("          data di partenza=" + t.getLeavingDate());
                    bw.newLine();
                    bw.write("          mittente=" + t.getSender().getName());
                    bw.newLine();
                    bw.write("          destinatario=" + t.getReceiver().getName());
                    bw.newLine();
                    bw.write("          numero di libri=" + t.getQuantity());
                    bw.newLine();
                    bw.write("          libri:");
                    bw.newLine();
                    int y = 0;
                    for (StandardBook b : t.getBooks().keySet()) {
                        bw.write("                  tipo di libro " + (y + 1) + ", isbn: " + b.getIsbn() + ", titolo "
                                + b.getTitle() + "; in quantità " + t.getBooks().get(b));
                        bw.newLine();
                        y++;
                    }
                }
    ...
    private boolean convertTextToPDF(File file) throws Exception {
        ...
        try {
            ...
            String output_file = file.getParent() + System.getProperty("file.separator")
    ...
    public static void main(String... args) {
        ...
        // r.sendEmail("ferocemarcello@gmail.com",
        // "ferocemarcello@virgilio.it");
        // r.sendEmail2("ferocemarcello@gmail.com",
        // "ferocemarcello@virgilio.it", "prova", "messagio di prova");
    }
}
